...
class FindFaceService:
    ...
    def extract_embeddings_in_db(self):
        ...
        while True:
            images_with_faces_without_embeddings = self.image_repository.get_images_with_faces_without_embeddings(1000)
            if not images_with_faces_without_embeddings:
                return
            for image_entity in images_with_faces_without_embeddings:
                image = cv2.imread(image_entity.path)
                for face in image_entity.faces:
                    ...
                    self.image_repository.save_embedding(image_entity.id, embedding)
